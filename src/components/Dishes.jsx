        ...
        <section className='w-full py-6' id='menu'>
            ...
            <div className='flex items-center gap-5 justify-center my-7 py-8 overflow-x-scroll scrollbar-none'>
                {categories && categories.map((item, index) => (
                    <div id={index} onClick={() => handleSelect(item.urlParamName)} className={` ${item.urlParamName === selectedItem ? "bg-cartNumBg" : "bg-card"}  group flex h-28 min-win-[94px] w-24 flex-col items-center justify-center  rounded-lg  drop-shadow-lg hover:bg-cartNumBg  cursor-pointer transition-all ease-in-out duration-200`}>
